...
property :metrics_params, Hash, required: true
...
action :create do
  ...
  execute 'Deploy Cluster Metrics' do
    command "#{node['cookbook-openshift3']['openshift_common_client_binary']} new-app --template=metrics-deployer-template --as=system:serviceaccount:openshift-infra:metrics-deployer #{metrics_params.map { |opt, value| " -p #{opt}=#{value}" }.join(' ')} -n openshift-infra --config=admin.kubeconfig"
    cwd Chef::Config[:file_cache_path]
